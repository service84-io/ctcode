...
int Main::RunMain(OmniPointer<s84::ctcode::system::ctcode::System> system, std::string ctcode_file_name, std::string transpiler)
{
    ...
    std::string ctcode = system->ReadFileToString(ctcode_file_name);
    if (ctcode==std::string(""))
    {
        logger->WriteLine(Concat(std::string("The file "),Concat(ctcode_file_name,std::string(" is empty or does not exist."))));
        return 1;
    }
    OmniPointer<s84::ctcode::dbnf::ctcode::LargeString> ctcode_large_string = std::shared_ptr<s84::ctcode::dbnf::ctcode::LargeString>(new s84::ctcode::dbnf::ctcode::LargeString());
