...
module CTCODE
module TRANSPILER
module NODEJSTRANSPILER
module CTCODE

class NodeJSTranspiler < ::S84::CTCODE::TRANSPILER::STANDARDSTRUCTURE::CTCODE::TargetSpecificFunctions
    def initialize()
        @system = nil
        @c_t_code_file = nil
        @base_name = ""
    ...
    def ProcessExdef(exdef)
        Append(@imports,Concat(Concat(Concat(Concat("import * as ",@string_helper.StripDot(exdef))," from \"./"),@string_helper.StripDot(exdef)),".js\""))
    end
    ...
    def BeginProcessingInterface(interface_name)
        ...
        Append(@interface_definitions,Concat(Concat("export class ",interface_name)," {"))
    end

    def ProcessInterfaceFunctionDeclaration(return_type, function_name, parameters)
        Append(@interface_definitions,Concat(Concat(Concat(@string_helper.Indentation(1),function_name),self.MakeParametersString(parameters))," {}"))
    end
    ...
    def BeginProcessingClass(class_name, implementing)
        ...
        Append(@class_definitions,Concat(Concat("export class ",class_name)," {"))
        ClearList(@class_init)
        ClearList(@class_functions)
        Append(@class_init,Concat(@string_helper.Indentation(1),"constructor() {"))
    end

    def BeginProcessingClassFunctionDefinition(return_type, function_name, parameters)
        Append(@class_functions,"")
        Append(@class_functions,Concat(Concat(@string_helper.Indentation(1),function_name),self.MakeParametersString(parameters)))
    end
    ...
    def ProcessDeclaration(indent, type, l_value, r_value)
        ...
        Append(@class_functions,Concat(Concat(Concat(Concat(@string_helper.Indentation(indent),"var "),l_value)," = "),r_value))
    end
    ...
    def ProcessClassMemberDeclaration(member_type, member_name)
        Append(@class_init,Concat(Concat(Concat(Concat(@string_helper.Indentation(2),"this."),member_name)," = "),self.GetDefault(member_type)))
    end
    ...
    def FinishProcessingCTCodeFile()
        destination_file_name = Concat(@string_helper.StripDot(@base_name),".js")
